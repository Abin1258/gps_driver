...
Eigen::Vector3d ENU_point;
 ...
 //gai
//std::ofstream out("/home/ysc/gps_driver/test.txt");
...
void publish_data(string data){
    ...
    try {
        ...
        // 经纬度和海拔   xiwangyuda 
        latitude_GPS = stod(ster[6]);//2
        longitude_GPS = stod(ster[7]);//4
        altitude_GPS = stod(ster[8]);//9
        // 当前解状态     0:无效解;  1:单点定位解;  2:伪距差分;  4:固定解;  5:浮动解;  6:组合导航解;  7:固定坐标;  9:WAAS 差分;
        // 当前解状态xingwang      3:单点定位解;   5:RTK; 8chun guan xing
        status = stod(ster[15]);//6
        // 当前卫星数量
        service = stod(ster[14]);//7
    } catch (int e) {
    ...
    //gai baocunweitxt
    if(1){
        std::ofstream out("/home/ysc/gps_driver/test.txt", std::ios::app);
        //out.clear();
        out<<fixed<<setprecision(7)<<longitude_GPS<<","<<latitude_GPS<<","<<altitude_GPS<<" ";
        out.close();
    }
    ...
    GPS_msg.altitude = altitude_GPS;
    GPS_msg.longitude = longitude_GPS;
...
int main(int argc, char** argv)
{
    ...
    GPS_pub = n.advertise<sensor_msgs::NavSatFix>("/gps/fix", 10);
